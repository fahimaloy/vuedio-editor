...
<template>
  ...
  <aside class="rounded-xl border border-white/10 bg-white/[0.03] p-4 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.6)]">
    ...
    <div class="flex items-center gap-3">
      <div class="flex items-center gap-2 shrink-0">
        ...
        >
          <PauseIcon v-if="playing" class="h-5 w-5" aria-hidden="true" />
        ...
        <!-- +5s -->
        <button
          @click="emit('seek', Math.min(duration||0, (currentTime||0)+5))"
          class="inline-flex size-9 items-center justify-center rounded-md border border-white/10 bg-white/5 text-neutral-200 hover:bg-white/10 active:scale-95 transition"
          title="Jump +5s"
        >
      ...
      <div class="ml-2 inline-flex items-center gap-2 rounded-md border border-white/10 bg-white/5 px-3 py-1 text-[12px] font-mono text-neutral-300 whitespace-nowrap">
    ...
    <div class="mt-3 pt-3 border-t border-white/10">
      <div class="flex items-center gap-3 min-w-0">
        <input
          ...
          :value="currentTime || 0"
          @input="handleSeek"
          class="min-w-0 w-0 flex-1 appearance-none bg-white/10 h-2 rounded-full outline-none"
        />
        <input
          type="number"
    ...
    <div class="pt-4">
      <div class="mb-3 flex items-center justify-between gap-2">
        <h3 class="text-sm font-semibold text-neutral-200">Effects &amp; Corrections</h3>
        <div class="flex flex-wrap items-center gap-2">
          ...
          >
            <SparklesIcon class="h-4 w-4 text-violet-400" aria-hidden="true" /> Grayscale
          </button>
          <button
            @click="addFilterPreset('sepia')"
            class="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-neutral-200 hover:bg-white/10 transition"
          >
            <BoltIcon class="h-4 w-4 text-amber-300" aria-hidden="true" /> Sepia
          </button>
          <button
            @click="addFilterPreset('brightness')"
            class="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-neutral-200 hover:bg-white/10 transition"
          >
            <AdjustmentsHorizontalIcon class="h-4 w-4 text-cyan-300" aria-hidden="true" /> Brightness
          </button>
        </div>
      </div>
        ...
        >
          <div class="mb-3 flex items-center justify-between">
            <div class="flex items-center gap-2 min-w-0">
              <span class="inline-flex size-2.5 rounded-full bg-violet-400/80"></span>
              <span class="text-sm font-medium text-neutral-200 truncate">{{ filter.label }}</span>
              <span class="text-[11px] text-neutral-400 hidden sm:inline">({{ filter.name }})</span>
            </div>
            <div class="flex items-center gap-2">
              <button
                ...
                title="Remove filter"
              >
                <XMarkIcon class="h-4 w-4" aria-hidden="true" /> Remove
              </button>
            </div>
          </div>
          ...
          <div v-if="filter.params" class="grid gap-3">
            ...
            >
              <!-- label -->
              <label class="col-span-12 sm:col-span-3 text-[12px] text-neutral-400">
                {{ key }}
              </label>

              <!-- slider + value pill -->
              <div class="col-span-9 sm:col-span-7 flex items-center gap-3 min-w-0">
                <input
                  type="range"
                  ...
                  :max="param.max"
                  :step="param.step || 1"
                  @input="emit('update-filter', index, key, param.value)"
                  class="min-w-0 w-0 flex-1 appearance-none bg-white/10 h-2 rounded-full outline-none"
                />
                <span class="min-w-12 text-right text-[11px] font-mono text-neutral-300">
              ...
              <div class="col-span-3 sm:col-span-2">
                <input
                  type="number"
                  v-model.number="filter.params[key].value"
                  :min="param.min"
                  :max="param.max"
                  :step="param.step || 1"
                  @input="emit('update-filter', index, key, filter.params[key].value)"
...
const toDisplay = (v) => (typeof v === 'number' && !Number.isInteger(v) ? v.toFixed(2) : v)
...
const addFilterPreset = (which) => {
  let preset
  if (which === 'grayscale') {
    preset = { name: 'grayscale', label: 'Grayscale', params: { intensity: { value: 100, min: 0, max: 100, step: 1 } } }
  } else if (which === 'sepia') {
    preset = { name: 'sepia', label: 'Sepia', params: { intensity: { value: 80, min: 0, max: 100, step: 1 } } }
  } else if (which === 'brightness') {
    preset = { name: 'brightness', label: 'Brightness', params: { level: { value: 1.0, min: 0.1, max: 3.0, step: 0.1 } } }
  } else {
    return
  }
  const f = JSON.parse(JSON.stringify(preset))
  f._id = (globalThis.crypto && crypto.randomUUID) ? crypto.randomUUID() : String(Date.now() + Math.random())
  emit('add-filter', f)
}
</script>
