...
<template>
  <div class="video-editor space-y-4">
    ...
    <!-- Export overlay -->
    <div v-if="exporting" class="fixed inset-0 z-50 grid place-items-center bg-black/50 backdrop-blur-sm">
      <div class="w-[320px] rounded-lg border border-white/10 bg-neutral-900/90 p-5 shadow-xl">
        <div class="mb-3 flex items-center justify-between text-xs text-neutral-300">
          <span>Exporting</span>
          <span>{{ exportProgress }}%</span>
        </div>
        <div class="h-2 w-full overflow-hidden rounded-full bg-white/10">
          <div class="h-full rounded-full bg-gradient-to-r from-violet-500 to-cyan-400 transition-all"
...
import initWasm, { VideoProcessor, export_video } from '../wasm/video_processor'
...
// Selection (shared between Preview <-> Timeline <-> Inspector)
const selection = ref([]) // [{trackId,itemId}]

/**
 * Tracks model:
 *  id, type: 'video'|'audio'|'text'|'image', label, items[]
 *  item: { id, start, end, label, x, y, w, h, z, kind?('video'|'image'|'text'|'audio'), src?, text?, color?, fontSize?, fontFamily? }
 */
const tracks = ref([
  { id: 't-video-1', type: 'video', label: 'Video 1', items: [] },
  { id: 't-audio-1', type: 'audio', label: 'Audio 1', items: [] },
  { id: 't-text-1',  type: 'text',  label: 'Text 1',  items: [] },
])
...
/* ---------- wasm boot ---------- */
onMounted(async () => {
  try { await initWasm() } catch (e) { console.error('WASM load failed', e) }
})
...
/* ---------- preview events ---------- */
const handleTimeUpdate = (t) => { currentTime.value = t }
const handleDurationChange = (d) => {
  duration.value = d
  ensureBaseClips()
}
const handleReady = () => { /* hook if needed */ }
...
/* ---------- transport ---------- */
const seekToTime = (t) => { preview.value?.setCurrentTime?.(t) }
const onTogglePlay = async (shouldPlay) => {
  const videoEl = preview.value?.getVideoElement?.()
  if (!videoEl) return
  try {
    if (shouldPlay) { await videoEl.play(); isPlaying.value = true }
    else { videoEl.pause(); isPlaying.value = false }
  } catch (e) { console.error('play/pause failed:', e) }
}

/* ---------- filters api ---------- */
const addFilter = (filter) => {
  if (!filter._id) filter._id = (crypto?.randomUUID ? crypto.randomUUID() : String(Date.now()+Math.random()))
  filters.value = [...filters.value, filter]
}
const removeFilter = (index) => {
  const next = filters.value.slice(); next.splice(index,1); filters.value = next
}
const updateFilter = (index, key, value) => {
  const f = filters.value[index]; if (!f) return
  const next = filters.value.slice()
  next[index] = { ...f, params: { ...f.params, [key]: { ...f.params[key], value } } }
  filters.value = next
}
...
const onAddTrack = (type) => {
  ...
  tracks.value.push({ id:`t-${type}-${Date.now()}`, type, label:`${type[0].toUpperCase()+type.slice(1)} ${n}`, items: [] })
}
...
const onPickClip = (e) => {
  ...
  const tr = findTrack(pendingClipTrackId) || tracks.value.find(t => isVideo ? t.type==='video' : t.type==='image') || tracks.value[0]
  ...
  tr.items.push({
    ...
    src: url,
    start, end,
    x: 0.1, y: 0.1, w: 0.4, h: 0.4, z: 0
  })
}

const onAddText = ({ trackId, start, end, text }) => {
  let t = trackId ? findTrack(trackId) : tracks.value.find(t => t.type === 'text')
  if (!t) { t = { id:`t-text-${Date.now()}`, type:'text', label:'Text', items:[] }; tracks.value.push(t) }
  t.items.push({
    id: newId(),
    kind: 'text',
    label: 'Text',
...
const onRemoveItem = ({ trackId, itemId }) => {
  const t = findTrack(trackId); if (!t) return
  t.items = t.items.filter(i => i.id !== itemId)
  selection.value = selection.value.filter(s => !(s.trackId===trackId && s.itemId===itemId))
}

const onSplit = ({ trackId, itemId, time }) => {
  const t = findTrack(trackId); if (!t) return
  const i = t.items.find(i => i.id===itemId); if (!i) return
  if (time<=i.start || time>=i.end) return
  const left  = { ...i, id:newId(), end:time }
  const right = { ...i, id:newId(), start:time }
  t.items = t.items.flatMap(it => it.id===itemId ? [left,right] : [it])
}
const onMerge = ({ trackId, itemIds }) => {
  const t = findTrack(trackId); if (!t) return
  const m = t.items.filter(i=>itemIds.includes(i.id)).sort((a,b)=>a.start-b.start)
  if (m.length<2) return
  const merged = { ...m[0], id:newId(), start:m[0].start, end:m[m.length-1].end, label:m[0].label||'Merged' }
  t.items = [...t.items.filter(i=>!itemIds.includes(i.id)), merged].sort((a,b)=>a.start-b.start)
}
const onCut = ({ start, end, selection: sel }) => {
  const byTrack = sel.reduce((m,s)=>((m[s.trackId]??=[]).push(s.itemId), m), {})
  for (const [trackId, ids] of Object.entries(byTrack)) {
    const t = findTrack(trackId); if (!t) continue
    const out=[]
    for (const it of t.items) {
      if (!ids.includes(it.id)) { out.push(it); continue }
      if (it.end <= start || it.start >= end) out.push(it)
      else {
        if (it.start < start) out.push({ ...it, id:newId(), end:start })
...
const onSelectionChange = (sel) => { selection.value = sel.slice() }

const onUpdateItem = ({ trackId, itemId, patch }) => {
  const t = findTrack(trackId); if (!t) return
  const idx = t.items.findIndex(i=>i.id===itemId); if (idx<0) return
  // clamp overlay geometry when present
  const p = { ...patch }
...
const onMoveItem = ({ itemId, fromTrackId, toTrackId, newStart }) => {
  const from = findTrack(fromTrackId); const to = findTrack(toTrackId)
  if (!from || !to) return
  const it = from.items.find(i=>i.id===itemId); if (!it) return
...
const exportVideo = async () => {
  ...
  try {
    if (!videoProcessor && videoEl.videoWidth && videoEl.videoHeight) {
      videoProcessor = new VideoProcessor(videoEl.videoWidth, videoEl.videoHeight)
    }
    const updateProgress = (p) => {
      const clamped = Math.max(0, Math.min(1, Number(p) || 0))
      exportProgress.value = Math.round(clamped * 100)
    }
    const blob = await export_video(videoEl, filters.value, tracks.value, duration.value, updateProgress)
    if (!(blob instanceof Blob)) {
      alert('Export not implemented in WASM yet.')
...
const onRemoveSelection = ({ trackId, itemId }) => onRemoveItem({ trackId, itemId })
const defaultsFor = (it) => {
  if (it.kind === 'text') {
    return { text: 'New Text', color:'#ffffff', fontSize:32, fontFamily:'Inter, ui-sans-serif, system-ui', x:0.2,y:0.2,w:0.4,h:0.15,z:10 }
  }
