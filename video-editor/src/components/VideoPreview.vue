...
<template>
  <section>
    <div v-if="videoUrl" class="relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.03] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.6)]">
      ...
      <div class="relative">
        <canvas ref="canvasElement" class="block w-full aspect-video bg-black"></canvas>
        ...
        <div ref="overlayRoot" class="absolute inset-0">
          <div
            ...
            :class="isSelected(it) ? 'ring-2 ring-violet-400/70' : ''"
            :style="overlayStyle(it)"
            @pointerdown.stop="startOverlayDrag(it,$event)"
            @click.stop="selectOverlay(it)"
          >
            <!-- media -->
            <video v-if="it.kind==='video' && it.src" :src="it.src" playsinline muted loop class="h-full w-full object-contain rounded-md border border-white/10 pointer-events-none"></video>
            <img v-else-if="it.kind==='image' && it.src" :src="it.src" class="h-full w-full object-contain rounded-md border border-white/10 pointer-events-none" />
            <!-- text -->
            <div v-else-if="it.kind==='text'"
                 class="grid h-full w-full place-items-center rounded-md border border-white/10 pointer-events-none px-2 text-center"
                 :style="textStyle(it)">
              {{ it.text || 'Text' }}
            </div>
        ...
        <!-- Buffer overlay -->
        <div v-if="showBuffer" class="absolute inset-0 grid place-items-center bg-black/30 backdrop-blur-sm">
          <div class="w-[320px] rounded-lg border border-white/10 bg-neutral-900/80 p-4 shadow-xl">
    ...
    <div v-else class="grid place-items-center rounded-xl border border-dashed border-white/10 bg-white/[0.02] py-16 text-center">
      <div class="max-w-md space-y-3 px-4">
        <div class="mx-auto grid size-12 place-items-center rounded-lg bg-gradient-to-br from-violet-600 to-cyan-400 text-neutral-950 font-extrabold">V</div>
        <p class="text-sm text-neutral-300">Upload a video to get started</p>
...
const emit = defineEmits(['time-update','duration-change','ready','playing-change','overlay-move','overlay-resize','overlay-select'])

const videoElement = ref(null)
const canvasElement = ref(null)
const overlayRoot = ref(null)
let ctx=null, raf=null, processor=null
...
const onLoadedData = () => { updateBufferUI(); emit('ready') }
const onCanPlayThrough = () => { bufferedPercent.value = 100; showBuffer.value = false }
...
const ensureProcessor = () => {
  if (!processor && width>0 && height>0) processor = new VideoProcessor(width, height)
}

/* Simple draw loop (base frame + optional filters via processor if you’ve wired them) */
const loop = () => {
  const v = videoElement.value
  const c = canvasElement.value
  if (!v || !c || !ctx) return
  ctx.drawImage(v, 0, 0, c.width, c.height)
  // (Optional) apply filters in WASM on the canvas.ImageData if you’ve implemented it.
...
const isSelected = (it) => props.selected?.some(s => s.itemId === it.id && s.trackId === it.trackId)
...
const onDragMove = (e) => {
  ...
  emit('overlay-move', { id: dragState.id, trackId: dragState.trackId, x: nx, y: ny })
}
const onDragEnd = () => {
  dragState = null
...
const startOverlayResize = (it, e) => {
  ...
  const pxToNormY = (px) => (px / rect.height)
  resizeState = {
    id: it.id, trackId: it.trackId,
    startX: e.clientX, startY: e.clientY,
    baseW: it.w || 0.1, baseH: it.h || 0.1,
    pxToNormX, pxToNormY
  }
...
const onResizeMove = (e) => {
  ...
  nw = Math.min(nw, 1)
  nh = Math.min(nh, 1)
  emit('overlay-resize', { id: resizeState.id, trackId: resizeState.trackId, w: nw, h: nh })
}
const onResizeEnd = () => {
  resizeState = null
...
/* expose to parent */
const setCurrentTime = (t) => { const v=videoElement.value; if (v) v.currentTime=t }
const getVideoElement = () => videoElement.value
defineExpose({ setCurrentTime, getVideoElement })

onMounted(()=>{ /* loop started on metadata */ })
